...
namespace DefferedRender
{
    public class GPUDravinDrawStack
    {
        ...
        /// <summary>        /// 渲染所有的需要特别获取反射数据的物体        /// </summary>
        public void DrawOtherSSS(ScriptableRenderContext context, CommandBuffer buffer, Camera camera)
        {
            if (queueStack == null) return;
            foreach (GPUDravinBase index in queueStack)
            {
                index.DrawOtherSSS(context, buffer, camera);
            }
        }
